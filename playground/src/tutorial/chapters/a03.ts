...
// この + 店 + が + 高い (基本形) + わけではない (い-adjective connects directly)
type この店が高いわけではない = \`\${PhraseWithParticle<\`\${この}\${店}\`, "が">}\${ConjugateAdjective<高い, "Basic">}わけではない\`;
`,
        },
      ],
    },
...
`,
        },
        {
          ...
          zh: "今天不能请假。",
          code: `import type { CommonNoun, PhraseWithParticle, GodanVerb, ConjugateVerb } from "typed-japanese";

type 今日 = CommonNoun<"今日">;
type 休む = GodanVerb & { stem: "休"; ending: "む" };
type わけ = CommonNoun<"わけ">;
type いく = GodanVerb & { stem: "い"; ending: "く" };

// 今日 + は + 休む(辞書形) + わけ + に + は + いく(否定丁寧 いきません)
type 今日は休むわけにはいきません = \`\${PhraseWithParticle<今日, "は">}\${ConjugateVerb<休む, "Dictionary">}\${PhraseWithParticle<わけ, "に">}は\${ConjugateVerb<いく, "Masen">}\`;
`,
        },
      ],
    },
